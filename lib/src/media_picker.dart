...
class _MediaPickerState extends State<MediaPicker> {
  ...
  Widget build(BuildContext context) {
    return Material(
      ...
      child: _albums==null ? LoadingWidget(decoration: widget.decoration,) : _albums.length==0 ? NoMedia() : Column(
        children: [
          if(decoration.actionBarPosition==ActionBarPosition.top) _buildHeader(),
          Expanded(
              child: Stack(
                children: [
                  Positioned.fill(child: MediaList(
                    album: selectedAlbum,
  ...
  _fetchAlbums() async{
    RequestType type;
    if(widget.mediaType==MediaType.all) type = RequestType.common;
    else if(widget.mediaType==MediaType.video) type = RequestType.video;
  ...
  void handleBackPress(){
    if(albumController.isPanelOpen) albumController.close();
