...
class _AlbumTileState extends State<AlbumTile> {
          ...
          child: Row(
            ...
            children: [
              Container(
                ...
                width: 80,
                height: 80,
                child: !hasError ? albumThumb!=null ? ClipRRect(
                  borderRadius: BorderRadius.circular(8),
                  child: Image.memory(
                    albumThumb,
                    fit: BoxFit.cover,
                  ),
                ) : LoadingWidget(decoration: widget.decoration,)
                    : Center(child: Icon(Icons.error_outline, color: Colors.grey.shade400, size: 40,),),
              ),
              SizedBox(width: 10,),
              Text(widget.album.name, style: TextStyle(color: Colors.black, fontSize: 18),),
              SizedBox(width: 5,),
              Text(
  ...
  _getAlbumThumb(AssetPathEntity album) async{
    ...
    if(_thumbByte!=null) setState(() => albumThumb = _thumbByte);
